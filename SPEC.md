...
- **Mobile**：宿主提供一个“插件中心”页面（可放在设置里或首页入口），同样列出所有 `pages`；可优先展示 `entry: true` 的页面
- **PC**：同 Mobile（插件中心）；并建议在顶栏主导航（如 `首页` / `喜欢`）旁额外展示少量 `entry: true` 的页面作为一级入口（例如最多 `3` 个，按 `order` 排序），超出部分仍在“插件中心”中
